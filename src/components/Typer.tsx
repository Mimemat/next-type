...
const Typer: React.VFC<{words: IWord[]}> = ({ words }) => {
  ...
  useEffect(() => {
    ...
    if (activeChar >= words[activeWord].value.length && pressed.code === 'Space') {
    ...
    <AnimateSharedLayout>
      <div className={`flex flex-col`}>
        ...
        <div className={`flex flex-row flex-wrap max-w-7xl items-center`}>
          ...
          {pastWords.map(word => 
            <p key={word.id} className={`text-2xl text-text-grey font-mono mr-6`}>{word.value}</p>
          )}
          ...
          {words[activeWord].value.split('').map((letter, index) => 
            <div className={`inline-flex`} key={`active:${letter}${index}`}>
              {index === activeChar &&
                <motion.span 
                  layoutId="cursor" 
                  className={`h-7 rounded-2xl w-0.5 bg-primary animate-blink font-thin text-2xl m-0`}
                  transition={{
                    type: "spring",
                    stiffness: 500,
                    damping: 30,
